...
class StochasticComputerExperiment(ComputerExperiment):
    ...
    def eval(self, x, simulated_noise_variance=True, batch_size=1):
        ...
        if simulated_noise_variance is not None:
            z = np.tile(
                super().eval(x).reshape(n, self.output_dim, 1),
                (1, 1, batch_size)
            ...
            for i in range(self.output_dim):
                if simulated_noise_variance[i] > 0.0:
                    z[:, i, :] = z[:, i, :] + np.random.normal(
                        loc=0.0,
                        scale=np.sqrt(self.simulated_noise_variance[i]),
                        size=(n, batch_size)
                    )
        else:
            raise RuntimeError("Dealing with true stochastic simulator is not implemented yet")
